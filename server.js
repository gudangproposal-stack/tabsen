...
// Access-Control headers for any external calls/CORS
...
const DEFAULT_STATE = {
  companies: [
    {
      id: 'comp_seed01',
      name: 'PT Sinar Jasa Abadi',
      address: 'Menara Astra, Jl. Jend. Sudirman Kav 5, Jakarta Pusat',
      logo_url: '',
      lat: -6.2088,
      long: 106.8456,
      radius_meter: 150,
  ...
  users: [
    {
      id: 'usr_admin',
      company_id: 'comp_seed01',
      role: 'admin',
      nama: 'Budi Gunawan (HR)',
      email: 'admin@hadir.id',
      nomor_hp: '081234567890',
      // password_hash for 'admin123' with salt '_hadir_salt_123!'
      // sha256 of 'admin123_hadir_salt_123!'
      password_hash: crypto.createHash('sha256').update('admin123_hadir_salt_123!').digest('hex'),
      jabatan: 'HR Manager',
    ...
    },
    {
      ...
      role: 'karyawan',
      nama: 'Rian Wijaya',
      email: 'karyawan@hadir.id',
      nomor_hp: '085712345678',
      // password_hash for 'karyawan123' with salt '_hadir_salt_123!'
      password_hash: crypto.createHash('sha256').update('karyawan123_hadir_salt_123!').digest('hex'),
      jabatan: 'Software Engineer',
      departemen: 'IT & Dev',
      foto_wajah_url: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100"><rect width="100%" height="100%" fill="%234f46e5"/><text x="50%" y="55%" font-family="sans-serif" font-size="28" fill="white" font-weight="bold" text-anchor="middle">RW</text></svg>',
      status: 'active',
    }
  ],
  invites: [
...
// Robust database helper with synchronous and file status check
function readDb() {
  try {
...
const db = {
  ...
  getCompany: (id) => readDb().companies.find(c => c.id === id),
  saveCompany: (company) => {
    const store = readDb();
  ...
  getInvites: () => readDb().invites,
  getInviteByToken: (token) => readDb().invites.find(i => i.token === token),
  saveInvite: (invite) => {
    const store = readDb();
  ...
  getAttendances: () => readDb().attendances,
...
const PLAN_PRICES = {
  ...
  'starter_yearly': { name: 'Starter Tahunan', plan: 'starter', limit: 50, amount: 2870400, cycle: 'yearly' },
  'pro_monthly': { name: 'Pro Bulanan', plan: 'pro', limit: 300, amount: 799000, cycle: 'monthly' },
  'pro_yearly': { name: 'Pro Tahunan', plan: 'pro', limit: 300, amount: 7670400, cycle: 'yearly' },
};
...
// WEBHOOK MIDTRANS REAL WITH SHA512 Verification
...
// Webhook simulation bypass (Midtrans sandbox tester UI handler)
app.post('/api/simulate/webhook-pay', (req, res) => {
  ...
  res.json({
    message: 'Simulasi webhook pembayaran berhasil diproses secara internal!',
    transaction: tx
  });
});
...
app.post('/api/employees/invite', authenticate, (req, res) => {
  ...
  const maxLimit = company.subscription_plan === 'pro' ? 300 : (company.subscription_plan === 'starter' ? 50 : 15);

  const { invites } = req.body;
  if (!invites || !Array.isArray(invites) || invites.length === 0) {
    return res.status(400).json({ error: 'Mohon masukkan data karyawan untuk diundang.' });
  }
  ...
  for (const emp of invites) {
    ...
    results.push({
      ...
      success: true,
      invite_url: `/activate/${inviteToken}`
    });
  }
...
// 5. EMPLOYEE ACTIVATION DETAIL & COMPLETION
...
app.post('/api/activation/complete', (req, res) => {
  ...
  invitedUser.foto_wajah_url = foto_wajah_base64;
...
// 6. PROCESS ATTENDANCE (CLOCK IN / CLOCK OUT)
...
app.post('/api/attendance/clock', authenticate, (req, res) => {
  const user = req.user;
  const { type, lat, long, selfie_base64 } = req.body;
...
// 7. ADMIN DASHBOARD SUMMARY & HISTORIES
...
app.get('/api/dashboard/history', authenticate, (req, res) => {
  ...
  const { start_date, end_date, employee_id, departemen } = req.query;
...
// FRONTEND SPA FALLBACK + STATIC SERVING
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`[Hadir.id Server] Single-file architecture bound at port ${PORT}`);
});
